<div dir="rtl">

# 📲 התקנה בטאבלט ושחרור גרסאות — המדריך המעשי

*(הרקע המלא — למה לא חנות, כללי חתימה — נמצא ב-[PUBLISHING.md](PUBLISHING.md))*

---

## חלק א — התקנה ראשונה של גרסת release בטאבלט (חד-פעמי!)

ההתקנה הראשונה מחליפה אפליקציה ישנה שחתומה במפתח אחר, ולכן אנדרואיד מחייב
הסרה — שמוחקת את כל הנתונים. לכן הסדר קריטי:

1. **גיבוי**: באפליקציה הישנה ← 🔒 מסך הורים ← טאב "הוספה" ← **ייצוא**.
   הטקסט מועתק ללוח — הדביקו אותו בפתק/קובץ ושמרו (למשל ב-Google Keep).
2. **הסרה**: הסירו את האפליקציה הישנה (לחיצה ארוכה על האייקון ← הסרה).
3. **העברת הקובץ**: את `kids-player-v<גרסה>.apk` (בתיקיית `apk_versions/`
   בפרויקט במחשב) העלו לגוגל דרייב או שלחו לעצמכם בוואטסאפ, ופתחו בטאבלט.
4. **התקנה**: הקישו על הקובץ. אנדרואיד יבקש אישור חד-פעמי
   **"התקנה מאפליקציות לא ידועות"** עבור דרייב/וואטסאפ — אשרו והתקינו.
5. **שחזור**: פתחו את האפליקציה ← מסך הורים ← **ייבוא** ← בחרו את קובץ הגיבוי.
   הכול חוזר: הרשימה, המחיקות, המתנות שכבר נפתחו.

> מכאן והלאה — עדכונים מגיעים **מתוך האפליקציה** בכפתור אחד. לא תצטרכו את
> ההליך הזה שוב.

### הגדרות מומלצות בטאבלט (חד-פעמי)
- **הצמדת אפליקציה** (הגדרות ← אבטחה ← App pinning) — הילד לא יוצא ממנה.
- **כיבוי מסך אחרי 5-10 דקות** — האפליקציה משאירה את המסך דולק בזמן ניגון
  בעצמה; ההגדרה קובעת מה קורה כשלא מנגנים.

---

## חלק ב — שחרור גרסה חדשה

### ⚡ הדרך המהירה — פקודה אחת

ב-macOS/Linux:

```bash
./release.sh
```

ב-Windows (PowerShell, מתיקיית הפרויקט):

```powershell
powershell -ExecutionPolicy Bypass -File release.ps1
```

זה כל מה שצריך כשהגרסה כבר קודמה (למשל מוזג PR עם קידום): הסקריפט מסנכרן את
main, מריץ טסטים, בונה APK חתום, **מאמת חתימה** (עוצר אם נכשל), יוצר את הקובץ
בשם הנכון בתיקיית `apk_versions/` ומפרסם release ב-GitHub. אם לחשבון ה-`gh`
הנוכחי אין הרשאת פרסום — הוא מדפיס הוראות פרסום ידני מדויקות (כולל הנתיב
המלא של הקובץ). הודעות הפלט של הסקריפט באנגלית.

וריאציות (זהות בשני הסקריפטים):

```bash
./release.sh patch                      # קידום 1.0.3 → 1.0.4 + כל השאר
./release.sh minor "הערות למה חדש"      # קידום 1.0.x → 1.1.0 עם הערות release
./release.sh 2.0.0                      # גרסה מפורשת
npm run release                         # שקול ל-./release.sh (במק)
npm run release:win                     # שקול ל-release.ps1 (ב-Windows)
```

`release.ps1` הוא פורט מלא של release.sh: אותם שלבים, אותן בדיקות עצירה, ואותה
תיקיית פלט `apk_versions/`. הוא מגדיר לבד `JAVA_HOME`/`ANDROID_HOME` לברירות
המחדל של Android Studio (אפשר לעקוף במשתני סביבה) ומאתר את `apksigner` העדכני
שמותקן. ה-keystore צריך להיות ב-`C:\Users\<שם>\.keystores\` (ראו למטה).

מה ששני הסקריפטים **לא** עושים: לדחוף את קומיט הקידום ל-GitHub (ב-flow של PRs
עושים את זה דרך PR).

> 💡 **איפה נוצר ה-APK בפועל?** ה-build של אנדרואיד תמיד כותב את הקובץ בשם הקבוע
> `android/app/build/outputs/apk/release/app-release.apk`. הסקריפט (או שלב 5 בדרך
> הידנית) **מעתיק** אותו לשורש הפרויקט בשם `kids-player-v<גרסה>.apk` — אותם בייטים
> בדיוק, רק בשם שכפתור העדכון באפליקציה יודע לחפש ב-GitHub. אפשר להעלות כל אחד
> מהשניים ל-release, כל עוד השם שמועלה הוא לפי התבנית `kids-player-v<גרסה>.apk`.

### שלב 0: הכנת מחשב לבנייה (חד-פעמי לכל מחשב)

**דרישות בכל מערכת הפעלה:**
1. **Node.js 18+** — nodejs.org
2. **JDK 17** — הכי קל: להתקין Android Studio שמביא אותו
3. **Android SDK** — מגיע עם Android Studio (להריץ אותו פעם אחת שיוריד את ה-SDK)
4. `npm install` בתיקיית הפרויקט
5. **ה-keystore** — ראו "העברת מפתח החתימה" למטה. בלעדיו build של release ייכשל
   (בכוונה!) עם הודעה ברורה.

**מיקומי ברירת מחדל:**

| | macOS | Windows |
|---|---|---|
| JAVA_HOME | `/opt/homebrew/opt/openjdk@17/...` או ה-JBR של Android Studio | `C:\Program Files\Android\Android Studio\jbr` |
| ANDROID_HOME | `~/Library/Android/sdk` | `%LOCALAPPDATA%\Android\Sdk` |
| keystore | `~/.keystores/` | `C:\Users\<שם>\.keystores\` |

**בנייה ב-macOS** — הסקריפטים המוכנים עובדים כמו שהם:

```bash
npm run apk:release
npm run apk:verify
```

**בנייה ב-Windows** — הסקריפטים ב-package.json מניחים macOS, אז בונים ישירות עם gradle
(אחרי שמגדירים `JAVA_HOME` ו-`ANDROID_HOME` כמשתני סביבה במערכת, או ב-PowerShell):

```powershell
$env:JAVA_HOME = "C:\Program Files\Android\Android Studio\jbr"
$env:ANDROID_HOME = "$env:LOCALAPPDATA\Android\Sdk"
npx cap copy android
cd android
.\gradlew.bat assembleRelease
# הפלט: android\app\build\outputs\apk\release\app-release.apk
# אימות חתימה:
& "$env:ANDROID_HOME\build-tools\34.0.0\apksigner.bat" verify --print-certs app\build\outputs\apk\release\app-release.apk
```

**העברת מפתח החתימה למחשב אחר (קריטי!):**
1. העתיקו את **שני** הקבצים מ-`~/.keystores/` במק — `kids-player-release.jks` +
   `kids-player.properties` — לתיקייה `C:\Users\<שם>\.keystores\` (או `~/.keystores` במק אחר).
   העבירו בדרך פרטית (USB / AirDrop) — לא במייל ולא ל-repo!
2. **ערכו את `kids-player.properties`** ועדכנו את השורה `storeFile=` לנתיב החדש, למשל:
   `storeFile=C:\\Users\\assaf\\.keystores\\kids-player-release.jks`
3. זהו — ה-gradle מוצא את הקובץ אוטומטית לפי `user.home`.
   ⚠️ לעולם אל תיצרו keystore חדש "כי לא בא לכם להעביר" — APK חתום במפתח אחר
   **לא יתקין כעדכון** על אף מכשיר קיים.

**קובץ נוסף שנדרש לבנייה:** `www/js/keys.local.js` (מפתח ה-YouTube API — מחוץ לגיט
כי ה-repo ציבורי). העתיקו גם אותו למחשב החדש, או צרו אותו מחדש:

```js
export const DEFAULT_YT_API_KEY = 'AIza...';
```

### שלב 1: בניית ה-APK (במחשב)

```bash
# 1. העלאת מספר הגרסה — מקור האמת היחיד (הכול נגזר ממנו אוטומטית)
npm version patch --no-git-tag-version        # 1.0.0 → 1.0.1 (או minor/major)

# 2. טסטים — חובה ירוק
npm test

# 3. בנייה חתומה
npm run apk:release

# 4. אימות חתימה — חובה לפני כל פרסום!
npm run apk:verify                             # חייב להדפיס "Verifies"

# 5. שם הקובץ לפי המוסכמה שהאפליקציה מחפשת, בתיקיית האיסוף apk_versions/
mkdir -p apk_versions
cp android/app/build/outputs/apk/release/app-release.apk apk_versions/kids-player-v1.0.1.apk
cp android/app/build/outputs/apk/release/app-release.apk apk_versions/kids-player.apk

# 6. שמירת הגרסה בגיט
git add package.json package-lock.json && git commit -m "release v1.0.1" && git push
```

### שלב 2: פרסום ב-GitHub — שתי דרכים

#### דרך א' — פקודה אחת (מטרמינל מחובר לחשבון devfassaf)

```bash
gh release create v1.0.1 apk_versions/kids-player-v1.0.1.apk apk_versions/kids-player.apk --repo devfassaf/kids_player -t "v1.0.1" -n "מה חדש: ..."
```

#### דרך ב' — דרך האתר (2-3 דקות)

1. גלשו אל **github.com/devfassaf/kids_player/releases/new**
   (או: עמוד ה-repo ← "Releases" בעמודה הימנית ← "Draft a new release").
2. **Choose a tag** ← הקלידו `v1.0.1` (בדיוק כך, עם v קטנה) ← לחצו על
   **"Create new tag: v1.0.1 on publish"** שמופיע. Target נשאר `main`.
3. **Release title**: `v1.0.1`.
4. **Describe this release**: מה חדש, בעברית — הטקסט הזה מוצג להורה במסך
   העדכון באפליקציה.
5. גררו את **שני** הקבצים — `apk_versions/kids-player-v1.0.1.apk`
   ו-`apk_versions/kids-player.apk` — לתיבת
   **"Attach binaries by dropping them here or selecting them"** והמתינו
   לסיום ההעלאה (פס התקדמות ואז שם הקובץ ברשימה).
   ⚠️ **שני השמות חשובים**: `kids-player-v<גרסה>.apk` היא התבנית שהאפליקציה מחפשת,
   ו-`kids-player.apk` הוא השם הקבוע שכפתור ההורדה באתר מצביע אליו
   (`/releases/latest/download/kids-player.apk`). בלי השני — הכפתור באתר יחזיר 404.
6. ודאו ש-**"Set as the latest release"** מסומן (ברירת המחדל).
7. **Publish release** (הכפתור הירוק). לא "Save draft" — טיוטה בלתי נראית לאפליקציה!

### שלב 3: קורה מעצמו

בכל טאבלט מותקן: בתוך עד 6 שעות (או מיידית בלחיצה על "בדיקת עדכון" במסך
ההורים ← הגדרות) יופיע "יש גירסה חדשה 🎉" ← "⬇️ עדכון גירסה" ← הורדה עם
אחוזים ← מסך ההתקנה של אנדרואיד ← מאשרים ← מעודכן. הנתונים נשמרים
(אותה חתימה = עדכון במקום, בלי הסרה).

### כללי ברזל

| כלל | למה |
|---|---|
| לעולם לא לאבד את `~/.keystores/` | אובדן = אף עדכון לא יותקן לעולם ([PUBLISHING.md](PUBLISHING.md)) |
| מספר גרסה תמיד עולה | אנדרואיד מסרב לשנמוך (`INSTALL_FAILED_VERSION_DOWNGRADE`) |
| תמיד לאמת חתימה לפני פרסום | APK לא חתום/חתום-debug יתקע את כל המכשירים |
| את ה-tag מקלידים ידנית באנגלית | tag שהודבק מטקסט עברי קיבל תו RLM בלתי-נראה ושבר את זיהוי הגרסה (קרה באמת ב-v1.0.0 הראשון) |
| בטא? `gh release create --prerelease` | האפליקציות מתעלמות מ-prerelease אוטומטית |

---

## שאלות נפוצות

**למה המכשיר שואל "לאשר התקנה ממקור לא ידוע" — ואפשר לבטל את זה?**

יש כאן שני דיאלוגים שונים, וחשוב להבדיל:

1. **אישור "התקנת אפליקציות לא ידועות"** (מסך הגדרות) — ההרשאה הזו ניתנת
   **פר-אפליקציה-מתקינה, פעם אחת**. אם התקנתם פעם דרך דרייב, פעם דרך ואטסאפ
   ופעם מהאפליקציה עצמה — כל אחת מהן ביקשה אישור משלה, ולכן זה "שאל כל פעם".
   **מרגע שמעדכנים רק דרך כפתור "עדכון גירסה" שבתוך האפליקציה, האישור נשמר
   אחרי הפעם הראשונה ולא יופיע שוב.**
2. **מסך "האם להתקין/לעדכן את האפליקציה?"** של אנדרואיד (לחיצת אישור אחת) —
   את זה **אי אפשר לבטל**, בכוונה: זו הגנת אבטחה בסיסית של המערכת, ורק אפליקציות
   מחנות Play או מכשירים בניהול ארגוני (MDM) עוקפים אותה. בפועל: עדכון = לחיצה
   אחת על "עדכון".

בנוסף, לפעמים **Play Protect** מציג "לסרוק את האפליקציה?" — אפשר ללחוץ
"אל תשלח"/"התקן בכל זאת"; זה בדרך כלל מפסיק להופיע אחרי הפעמים הראשונות.

</div>
