...
> **Google Sheets API** אינו נדרש לפרויקט חדש. בפרויקט קיים הוא נחוץ רק עד **10.9.2026**:
> כל עוד קוד המיגרציה חי הוא קורא את הגיליון הישן פעם אחרונה דרך ה-API הזה. אחרי אותו
> תאריך (`test/sunset.test.mjs` נופל בו ומזכיר) אפשר לכבות אותו.
